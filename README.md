...
The service returned via the call to ```createService()``` will have any invocations to methods annotated with @Transactional wrapped in a transaction, same as if you were to execute them inside of a call to SQLPlus.transact(). To execute work against the current session wrapping the method, you can call ```SQLPlus.getCurrentSession()```, as shown, on the injected ```sqlPlus``` instance.
